 ...
 * @param {string} pieceId - The piece identifier (e.g., "red1", "blue2")
 * @param {number} diceValue - The value of the selected die
 * @param {Object} routes - The routes object containing paths for all players
 * @returns {Object} Object containing:
 *   - success: boolean indicating if move was successful
 *   - message: string explaining failure reason if unsuccessful
 *   - hasWon: boolean indicating if player won (only if success is true)
 */
export const movePiece = (player, pieceId, diceValue, routes) => {
  ...
  if (piece.inHome) {
    ...
    const tileOccupied =
      gameState[player].filter(
        (p) =>
          p.coord?.row === startingTile.row && p.coord?.col === startingTile.col
      ).length >= 2;
  ...
  if (isOpponentOnSafeSpace(newCoord, player)) {
    return {
      success: false,
      message: "Cannot move to safe space occupied by opponent",
    };
  }
